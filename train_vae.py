    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Train]") as pbar:
            for batch_idx, frames in enumerate(pbar):
                ...
                # VAE forward pass
                recon, posterior, latent = model(frames, sample_posterior=True)
                ...
                if batch_idx % 100 == 0:
                    with torch.no_grad():
                        ...
                        batch_recon, _, _ = model(frames, sample_posterior=False)
        ...
        with torch.no_grad():
            with tqdm(val_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Val]") as pbar:
                for batch_idx, frames in enumerate(pbar):
                    ...
                    # VAE forward pass
                    recon, posterior, latent = model(frames, sample_posterior=False)
