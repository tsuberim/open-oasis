...
class GaussianPyramidLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        # Create lists to hold the pyramid levels
        pyramid_pred = [y_pred]
        pyramid_true = [y_true]

        # Generate the Gaussian pyramids
        for _ in range(self.max_level - 1):
            pyramid_pred.append(kornia.geometry.transform.pyrdown(pyramid_pred[-1]))
            pyramid_true.append(kornia.geometry.transform.pyrdown(pyramid_true[-1]))

        total_loss = 0
        # Calculate loss at each level with correct weighting
        for i in range(self.max_level):
            # Weight is HIGHEST for level 0 (full resolution) and decreases
